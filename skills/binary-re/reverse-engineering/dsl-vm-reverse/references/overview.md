...
Use this skill when the target file matches **any** of the following features:

| # | Feature | Description |
|---|------|------|
| 1 | IIFE at the start + many single-letter variable names | `!function(){var U=void 0,y=parseInt,E0=Function,...}` |
...
| File contains the WASM magic word via `Uint8Array([0,97,115,109])` | Embedded WASM | Extract the .wasm then go to IDA/Ghidra |
...
```javascript
// Feature 1: IIFE entry, single-letter variables mapped to numeric constants
!function(){
    var U=void 0, y=parseInt, E0=Function, AN=Uint8Array;
    var E=15, l=10, m=12, x=16, S=13, $=11;
    // Numeric constants mapped to variable names, replacing the original numbers
    ...
}

// Feature 2: interpreter main loop DG()
function DG(C, d, ...) {
    var d = [];  // Array emulating WASM stack/locals
    for (d[7] = x; d[7] !== U;) {
...
// Feature 3: constant table C[9] stores function indices and strings
...
// All built-in functions are called via the C[index] index
...
3. Find the object returned by the factory function → the exported function definition
...
| 21 | **STORE** | Safely obtain a global/object property |
...
> Tokens generated by the DSL VM are usually strongly bound to the browser context (TLS JA3 fingerprint, IP, Cookie, request headers, etc.); outside the browser the server can detect the context mismatch. **Pure protocol approaches are not recommended**.
...
| **Toolchain**: Playwright / Selenium CDP | Browser injection option |
