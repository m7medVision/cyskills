...
> For the complete OLLVM deobfuscation workflow, variant ecosystem, and community tool research, see [ollvm-deobfuscation.md](references/ollvm-deobfuscation.md).

### d810-ng (IDA) — local first choice

The modern maintained version of D-810 (Next Generation), integrating the **Z3 SMT** solver, covering OLLVM / Tigress / Hodur(PlugX) / Approov variants.
...
- Hacker's Delight bitwise equivalences
- PlugX (Hodur) malware-specific MBA patterns

Installation: clone → copy to the IDA plugins directory → Ctrl-Shift-D to load
Source: https://github.com/w00tzenheimer/d810-ng
```

### obpo-plugin (IDA) — strongest effect, cloud plugin

Based on Hex-Rays **microcode** + dataflow tracking + program slicing + concolic execution. Recognized by the community as one of the strongest.

```text
- Optimizes decompiler output at the microcode layer (not modifying ASM)
- Supports IDA 7.5-7.7 + Hex-Rays, multiple architectures (ARM/ARM64/x86/x64/PPC/MIPS)
- Cloud plugin: target functions are uploaded to obpo-server for processing (core closed source, plugin free)
- ⚠️ Use with caution on sensitive samples (binary uploaded to a cloud service)
- Usage: right-click dispatcher → OBPO → Mark and process function
Source: https://github.com/obpo-project/obpo-plugin
```

### ollvm-unflattener — Miasm pure scripting

No IDA dependency, based on Miasm symbolic execution, BFS multi-layer processing, supports x86/x64 Win/Linux.

```bash
git clone https://github.com/cdong1012/ollvm-unflattener
pip install -r requirements.txt   # miasm, graphviz, keystone-engine
python unflattener -i <input> -o <output> -t <func_addr> -a   # -a automatic multi-layer
```

### ollvm-breaker (Binary Ninja)

Binary Ninja deflattening; the repository ships with the Android-hardened sample `libvdog.so` for practice. Source: amimo/ollvm-breaker

### DeObfBR — BR obfuscation specialty

Specifically removes Goron/Arkari-style BR (indirect branch) obfuscation. Simple trick: making the data segment read-only partially counters it. Source: Mrack/DeObfBR

### deollvm — ARM64 Unicorn

A fallback for handling ARM64 .so without IDA, based on Unicorn. Source: GeT1t/deollvm
