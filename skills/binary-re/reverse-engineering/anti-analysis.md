...
> For the complete OLLVM deobfuscation workflow, variant ecosystem (Hikari/Polaris/O-MVLL/Tigress/Hodur, etc.), and community tool research, see [ollvm-deobfuscation.md](references/ollvm-deobfuscation.md).
...
**Modern variants (community-active in 2026):** Hikari (Anti Class Dump/String Encryption/Indirect Branch), Polaris (formerly Pluto, includes Trap Angr specifically to trap angr), O-MVLL (Python-driven, commonly used for Android hardening), Arkari (based on goron; indirect jumps can be countered by making the data segment read-only), amice (Rust, includes VM Flatten requiring VM reverse engineering rather than deflat). For variant identification see section 1 of ollvm-deobfuscation.md.

**Deobfuscation tools (ordered by community activity):**
- **obpo-plugin** (629⭐, IDA microcode+concolic cloud plugin, strongest effect): https://github.com/obpo-project/obpo-plugin
- **ollvm-breaker** (441⭐, Binary Ninja, Android .so in practice): https://github.com/amimo/ollvm-breaker
- **ollvm-unflattener** (265⭐, Miasm symbolic execution, pure scripting x86/x64): https://github.com/cdong1012/ollvm-unflattener
...
- **DeObfBR** (96⭐, BR indirect branch obfuscation specialty): https://github.com/Mrack/DeObfBR
- **D-810** (original, now less maintained, recommend d810-ng): pattern-based deobfuscation, MBA simplification
- **Miasm**: Symbolic execution for deobfuscation
- **Arybo** / **SiMBA**: MBA expression simplification

```bash
# d810-ng: copy to the IDA plugins directory, Ctrl-Shift-D to load
# Select Unflattener + MBA simplification + Opaque predicate removal
# obpo: right-click dispatcher → OBPO → Mark and process function (needs network)
# ⚠️ Pluto/Polaris's Trap Angr pass breaks angr → switch to d810-ng/Unicorn
```
...
> For the longform detection content see `malware-analysis/references/anti-analysis-techniques.md`; for the long OLLVM workflow see `references/ollvm-deobfuscation.md`.  
> This section is a short **trigger → action → Evidence** recipe list for the agent to use as Dynamic/Static bypass options.  
> **Authorized isolated lab is the default**; static patching / changing the PEB / changing return values are not default actions on unauthorized targets.

### Usage rules

1. First **identify and record** the detection point (address/API/string), then decide whether to bypass or change environment.  
2. Bypass attempts (successful or failed) MUST be written to Evidence; forbidden to write an anti-debug exit as "sample harmless".  
3. H/S do not expand into long text here → jump to the OLLVM chapter.  
4. L is mandatory only on Linux/ELF; the Windows PE main path does not fail due to a missing TracerPid.  
5. E's VT comparison is **optional**; when there is no external intelligence source write n/a and do not fabricate a first-submission time.

### Full table A–T

| ID | Trigger | Handling action | Evidence | Priority |
|----|------|----------|----------|--------|
| **A** | Conditional jump after `cpuid` (jz/jnz) | Identify hypervisor detection; in the lab change the flag or patch the jump to take the malicious/real business branch; or switch to a physical machine | `E-anti-debug-cpuid` | P0 |
| **B** | `rdtsc` + sub/cmp time difference | bp `rdtsc` / hook the time API; or patch the comparison; avoid relying only on "waiting for the sandbox to time out" | `E-anti-debug-rdtsc` | P0 |
| **C** | Strings contain x64dbg/olly/windbg etc., or Toolhelp enumeration | bp `CreateToolhelp32Snapshot`→`Process32First/Next`; change the match or skip the scanning branch | `E-anti-debug-procscan` | P1 |
| **D** | `AddVectoredExceptionHandler` + deliberate access violation | bp the registration point; locate and analyze the VEH handler; the debugger can ignore specific exceptions | `E-anti-debug-veh` | P1 |
| **E** | TimeDateStamp in the future/0/absurd; version info resembles a legitimate vendor | Compare with the discovery time; **optional** VT first submission; `SigCheck` to see whether version resources match the signature; no VT → n/a | `E-meta-timestamp` | P2 |
| **F** | Appears digitally signed but of suspicious origin | `SigCheck`: is the chain valid? revoked? signing time vs compile time; **invalid/revoked must not lower** the threat level | `E-sig-forge` | P0 |
| **G** | Multiple PE headers, overlapping sections, disguised section names | CFF/PE-bear/LoadPE to see the real mapping and EP section; use entropy to distinguish encryption vs code; don't trust section names | `E-pe-anomaly` | P1 |
| **H** | F5 shows lots of `while(1)+switch`, star-shaped CFG | **See** `ollvm-deobfuscation.md` (d810/deflat etc.); if plugins are insufficient, dynamically record the block order and reconstruct | `E-cff` | P1 pointer |
| **I** | strings have no domains/IPs but there is network/file behavior | Find Base64/XOR/custom decode; xref the decryption function; after decryption dump and re-inject into IDA | `E-string-decrypt` | P0 |
| **J** | File size ≫ sum of section raw data (Overlay) | Extract the overlay; `file`/entropy; search IDA for offset references; if encrypted, dynamically capture the key | `E-overlay` | P1 |
| **K** | `fs:[0x30]`/`gs:[0x60]` → BeingDebugged / NtGlobalFlag | Change the PEB flag or use ScyllaHide; or patch the conditional jump | `E-anti-debug-peb` | P0 |
| **L** | Reads `/proc/self/status` to check TracerPid≠0 | **Linux/ELF**: hook fopen/read or patch; not mandatory on Windows | `E-anti-debug-tracerpid` | P2 platform |
| **M** | `int3`(0xCC) or reads DR0–DR7 | int3→nop; for hardware BP detection use ScyllaHide/software BPs; for CRC self-checks see patch 6 | `E-anti-debug-bp` | P1 |
| **N** | Empty/very few IAT + self-written hash-based API resolution | bp `GetProcAddress`/`Ldr*`; reverse-lookup the hash in the export table; re-inject symbols; coordinate with the "clean IAT" iron rule | `E-api-hash` | P0 |
| **O** | Linear disassembly has lots of db, junk instructions cause misalignment | F5/Hex-Rays; dynamically confirm the real flow; after junk nops reanalyze; if static is incomplete, dynamic is authoritative | `E-junk-code` | P2 |
| **P** | `NtQueryInformationProcess` class 7/30/31 | ScyllaHide or hook the return; record the InformationClass | `E-anti-debug-ntqip` | P0 |
| **Q** | `.rsrc` too large/high entropy/non-standard RT_RCDATA | Extract with Resource Hacker/CFF; xref `FindResource`/`LoadResource`; after decryption dump | `E-rsrc-payload` | P1 |
| **R** | Static IAT lacks a DLL, used only at runtime | Check the Delay Import Table; bp `__delayLoadHelper2` or the first call; include in the capability assessment | `E-delay-import` | P1 |
| **S** | Always-true/always-false conditions, large blocks of dead code | **See** ollvm / angr etc.; patch the single reachable branch or re-inject the dynamic path | `E-opaque-pred` | P1 pointer |
| **T** | ASCII strings yield nothing, the data region looks like UTF-16 | `strings -el` or `-encoding=utf-16le`; IDA Alt+A unicode; include in the IOC | `E-wide-strings` | P0 |

### Connection with the main workflow

| Stage | Recipe |
|------|------|
| Triage | E, F, G, T (metadata/signature/section/wide strings) |
| Static | H, I, J, N clues, O, Q, R, S |
| Dynamic | A–D, K, L, M, N, P + the existing four-stage breakpoint rocket and no-behavior contingency |
| Failure | Any unbypassable detection → Evidence + switch tools/environment; do not silently lower the threat |

### Tool notes (not a mandatory install list)

- Windows user mode: x64dbg + **ScyllaHide** (bulk hiding of PEB/NtQuery/hardware BP etc.)  
- Signatures: Sysinternals **SigCheck**  
- PE structure: PE-bear / CFF Explorer  
- Flattening: see the OLLVM chapter's tool table (d810-ng etc.)  
- When a tool is unavailable: use equivalent commands + record the failure; forbidden to pretend the signature was verified / flattening was removed