...
- 如果要长期复用算法链路，这一阶段必须做
- Issue #65 混淆旁路（U–AV §4）：JSVMP（AD）→ `E-js-vmp`；CFF+字符串数组（AE）→ `E-js-deobf`；DevTools/debugger 反调试（AF）→ `E-js-anti-debug`。完整触发表见 `nonpe-format-cookbook.md`；AST 细节仍用 `ast-deobfuscation.md`
...
- 失败时按 `fallbacks.md` 回退
- 输出遵循 `output-contract.md`

## 必读引用

- 自动化入口：`automation-entry.md`
- 参数默认值：`tool-defaults.md`
- 任务输入模板：`task-input-template.md`
- MCP 专用任务编排：`mcp-task-template.md`
- 任务产物：`task-artifacts.md`
- 本地复现：`local-rebuild.md`
- 补环境：`env-patching.md`
- Node 复现：`node-env-rebuild.md`
- 插桩：`instrumentation.md`
- AST 去混淆：`ast-deobfuscation.md`
- 非 PE/JS 混淆菜谱 U–AV：`nonpe-format-cookbook.md`（AD/AE/AF）
- 回退：`fallbacks.md`
