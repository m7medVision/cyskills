...
| 任务类型 | 使用模板 |
|---------|---------|
| APK/二进制/so 逆向 | `security-report-templates.md` → 逆向工程报告 |
| 渗透测试/漏洞挖掘 | `security-report-templates.md` → 渗透测试报告 |
| JS/Web 签名逆向 | `security-report-templates.md` → 签名逆向报告 |
| 恶意软件 / APT / 病毒分析报告 | `security-report-templates.md` + **`vendor-report-rules.md`** |
| 通用技术文档 | `templates.md` → README / API 文档 |

### 厂商报告结构（Issue #65）

安全类正式报告 **MUST** 读取 `vendor-report-rules.md`（只取结构，不抄厂商原文）。仅在任务证据或用户明确要求时选择厂商 flavor；普通逆向和其他任务使用 `flavor = null`。
...
- **MUST** 包含 Evidence → Finding → Path 链（见 `evidence-finding-path.md` 与模板 §0）
- **MUST** 读取 `vendor-report-rules.md`：选定 `malware` / `apt` 或 `flavor = null`（漏洞任务可叠加 thin `vuln`）；无 flavor 时只输出原任务模板和适用的 Base 元素，不强制 IOC/ATT&CK
- **SHOULD** 引用 case scope.md / timeline.md（`step-cyskills scope gate`）
...
**安全报告模板**: `security-report-templates.md`
**厂商报告规则**: `vendor-report-rules.md`（flavor: malware | apt | null；optional overlay: vuln）
**通用文档模板**: `templates.md`
