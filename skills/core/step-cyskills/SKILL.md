---
name: step-cyskills
description: Bootstrap a real-work cybersecurity engagement in a new project. Initializes the git workspace, builds CONTEXT.md, enforces the authorization scope gate, detects the Linux distro, audits required tools and wordlist resources, and lists the skills this task needs so the user can install them. Run once per project before any security work; do not use for CTF challenges.
...
5. Skill selection: `bash scripts/recommend.sh "<task>"` lists the skills this task needs. Tell the user to install those skills before continuing; this skill never installs other skills itself.
